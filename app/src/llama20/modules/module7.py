...
class RepAwareUPUTrainer:
    ...
    def train(self):
        ...
        run_meta = {
            ...
            "capsules_dir": self.cfg.capsules_dir,
            "interactions_log": self.cfg.interactions_log,
            "out_dir": str(self.cfg.out_dir),
            "adapter_name_prefix": self.cfg.adapter_name_prefix,
            "seed": self.cfg.seed,
            ...
            "min_subject_pairs": self.cfg.min_subject_pairs,
            "min_anchor_pairs": self.cfg.min_anchor_pairs,
            "variants_per_subject": self.cfg.variants_per_subject,
            "max_subjects": self.cfg.max_subjects,
            "lora_r": self.cfg.lora_r,
            "lora_alpha": self.cfg.lora_alpha,
            "lora_dropout": self.cfg.lora_dropout,
            "target_modules": self.cfg.target_modules,
            "dpo_beta": self.cfg.dpo_beta,
            "unlikelihood_weight": self.cfg.unlikelihood_weight,
            "name_ul_weight": self.cfg.name_ul_weight,
            "ewc_lambda": self.cfg.ewc_lambda,
            "retain_mix": self.cfg.retain_mix,
            "kl_lambda": self.cfg.kl_lambda,
            "sig_suppression_strength": self.cfg.sig_suppression_strength,
            "use_4bit": self.cfg.use_4bit,
            "device": self.cfg.device,
            "dtype": str(self.cfg.dtype),
        }
        (save_dir / "module7_run_config.json").write_text(
            json.dumps(run_meta, indent=2), encoding="utf-8")
        result = {"adapter_path": str(save_dir), "run_config": run_meta}
        (self.cfg.out_dir / "last_adapter_path.json").write_text(
...
def run_module7_repaware(cfg: Optional[M7QwenConfig] = None):
    if cfg is None:
        cfg = M7QwenConfig()
...
run_module7_qwen = run_module7_repaware


def _parse_args():
    import argparse
    parser = argparse.ArgumentParser(
        description="Module 7 — Representation-Aware UPU LoRA training")

    parser.add_argument("--model_dir", type=str, default="outputs/model")
    parser.add_argument("--capsules_dir", type=str, default="outputs/capsules")
    parser.add_argument("--interactions_log", type=str, required=True)
    parser.add_argument("--out_dir", type=str, required=True)
    parser.add_argument("--adapter_name_prefix", type=str,
                        default="unlearning_adapter_repaware")

    parser.add_argument("--seed", type=int, default=17)
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--batch_size", type=int, default=2)
    # Kept for config reproducibility. The original implementation's optimizer
    # stepping semantics are intentionally left unchanged in this sweep build.
    ...
    parser.add_argument("--dpo_beta", type=float, default=0.02)
    parser.add_argument("--unlikelihood_weight", type=float, default=0.03)
    parser.add_argument("--name_ul_weight", type=float, default=0.02)
    parser.add_argument("--ewc_lambda", type=float, default=5.0)
    parser.add_argument("--retain_mix", type=float, default=0.60)
    parser.add_argument("--kl_lambda", type=float, default=0.03)
    parser.add_argument("--sig_suppression_strength", type=float, default=1.0)

    parser.add_argument("--device", type=str, default=None,
                        help="Usually omit. With CUDA_VISIBLE_DEVICES set per worker, 'cuda' maps to that worker's GPU.")
...
def _main_cli():
    ...
    cfg = M7QwenConfig(
        ...
        min_subject_pairs=args.min_subject_pairs,
        min_anchor_pairs=args.min_anchor_pairs,
        ...
        name_ul_weight=args.name_ul_weight,
        ewc_lambda=args.ewc_lambda,
        retain_mix=args.retain_mix,
        kl_lambda=args.kl_lambda,
        sig_suppression_strength=args.sig_suppression_strength,
        use_4bit=not args.no_4bit,
        device=device,
        seed=args.seed,
    )
    return run_module7_repaware(cfg)
