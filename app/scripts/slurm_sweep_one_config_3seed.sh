...
# Optional environment activation. Set KIF_CONDA_ENV or KIF_VENV_ACTIVATE before sbatch if needed.
...
# Respect Slurm's allocated GPU list. Do not assume physical GPU IDs are 0,1,2.
...
if [[ ${#ALLOCATED_GPUS[@]} -lt 3 ]]; then
  echo "[ERROR] Need 3 allocated GPUs, got: ${CUDA_VISIBLE_DEVICES:-unset}" >&2
  exit 1
fi
...
echo "[INFO] Finished 3 seeds for ${CONFIG_NAME}."
