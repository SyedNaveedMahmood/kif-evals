...
class M7QwenConfig:
    ...
    elude_upu_file: str = os.getenv("ELUDE_UPU_FILE", "outputs/elude/elude_upu_train.jsonl")
    elude_retain_file: str = os.getenv("ELUDE_RETAIN_FILE", "outputs/elude/elude_retain_train.jsonl")
    elude_world_train_file: str = os.getenv("ELUDE_WORLD_TRAIN_FILE", "data/alpaca_gpt4_data_train.json")
    elude_use_retain_anchors: bool = os.getenv("ELUDE_USE_RETAIN_ANCHORS", "1") not in {"0", "false", "False"}
    elude_use_world_anchors: bool = os.getenv("ELUDE_USE_WORLD_ANCHORS", "1") not in {"0", "false", "False"}
...
def _read_elude_upu_rows(path: str) -> List[Dict[str, Any]]:
    p = Path(path)
    if not p.exists():
        return []
    ...
    for line in p.read_text(encoding="utf-8").splitlines():
        ...
        try:
            rec = json.loads(line)
            if rec.get("subject") and rec.get("prompt"):
                rows.append(rec)
        except Exception:
            pass
    return rows
...
def _chat_text(tok, prompt: str, response: Optional[str] = None, add_generation_prompt: bool = False) -> str:
...
def _read_json_or_jsonl(path: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(f"[ELUDe] Could not read {path}: {e}")
    return []
...
def _generate_batch(model, tok, prompts: List[str],
    ...
    formatted = [_chat_text(tok, p, response=None, add_generation_prompt=True) for p in prompts]
...
def _build_subject_pairs(cfg: M7QwenConfig, tok, base_model,
    ...
    # ELUDe mode: train on the exact forget-train QA prompts prepared by
    # Module ELUDe. This avoids prompt-distribution mismatch between UPU and
    # Module 8E evaluation.
    elude_rows = [r for r in _read_elude_upu_rows(cfg.elude_upu_file)
                  if str(r.get("subject")) in set(subjects)]
    pairs = []
    n_capsule, n_fallback, n_answer, n_generated = 0, 0, 0, 0
    if elude_rows:
        logger.info(f"[Pairs] ELUDe mode enabled from {cfg.elude_upu_file}: {len(elude_rows)} rows")
        random.shuffle(elude_rows)
        for rec in elude_rows:
            p = str(rec.get("prompt"))
            subj = str(rec.get("subject"))
            # y_bad should be the factual target answer for the ELUDe question.
            # If the row lacks it, fall back to base-model generation.
            yb = str(rec.get("answer") or "").strip()
            if not yb:
                yb = _generate_batch(base_model, tok, [p], cfg, max_new_tokens=40)[0]
                n_generated += 1
            else:
                n_answer += 1
            if not yb or not yb.strip():
                continue
        ...
        logger.info("[Pairs] Generic-template fallback mode enabled")
        prompts, mapping = [], []
        for s in subjects:
    ...
    logger.info(
        ...
        f"y_bad sources: elude_answer={n_answer}, generated={n_generated}")
    # In ELUDe mode, keep the full selected forget set; do not silently truncate
    # to the generic KIF minimum-pair budget. Exact ELUDe evaluation expects the
    # whole target forget set to be optimized and then evaluated.
    if elude_rows:
        return pairs
...
def _sample_rows(rows: List[Dict[str, Any]], n: int, seed: int) -> List[Dict[str, Any]]:
    rows = [r for r in rows if _question_text(r) and str(r.get("answer") or "").strip()]
    if not rows or n <= 0:
        return []
    ...
    if len(rows) >= n:
        return rows[:n]
    # Upsample only if a split is unexpectedly smaller than the forget set.
    out = list(rows)
    while len(out) < n:
...
def _build_elude_anchor_pairs(cfg: M7QwenConfig, n_subject_pairs: int) -> List[Dict[str, Any]]:
    pairs: List[Dict[str, Any]] = []

    if cfg.elude_use_retain_anchors:
        retain_rows = _read_json_or_jsonl(cfg.elude_retain_file)
        for row in _sample_rows(retain_rows, n_subject_pairs, cfg.seed + 71):
            pairs.append({
                "prompt": _question_text(row),
                "y_good": str(row.get("answer") or "").strip(),
                "y_bad": _good_refusal_no_name(),
                "is_anchor": True,
                "anchor_type": "elude_retain",
            })

    if cfg.elude_use_world_anchors:
        world_rows = _read_json_or_jsonl(cfg.elude_world_train_file)
        for row in _sample_rows(world_rows, n_subject_pairs, cfg.seed + 173):
            pairs.append({
                "prompt": _question_text(row),
                "y_good": str(row.get("answer") or "").strip(),
                "y_bad": _good_refusal_no_name(),
    ...
    if pairs:
        n_retain = sum(1 for p in pairs if p.get("anchor_type") == "elude_retain")
        n_world = sum(1 for p in pairs if p.get("anchor_type") == "world")
        logger.info(f"[Pairs] ELUDe anchors: retain={n_retain}, world={n_world}")
    return pairs
...
def _sequence_logprob(model, tok, prompt: str, response: str,
    ...
    """Sequence log-probability using the model's chat template.

    Llama-3.1-Instruct is chat-tuned, and ELUDe evaluation uses chat-format
    user/assistant turns. Training the UPU pair in the same format avoids an
    avoidable train/eval prompt mismatch.
    """
    ctx = torch.enable_grad() if require_grad else torch.no_grad()
    with ctx:
        prompt_text = _chat_text(tok, prompt, response=None, add_generation_prompt=True)
        full_text = _chat_text(tok, prompt, response=response, add_generation_prompt=False)
        ...
        attn = enc_full.get("attention_mask", torch.ones_like(ids))
        labels_full = ids.clone()
        prompt_len = min(enc_prompt["input_ids"].shape[1], labels_full.shape[1])
        labels_full[:, :prompt_len] = -100
        labels = labels_full[:, 1:]
        logits = model(input_ids=ids, attention_mask=attn).logits[:, :-1, :]
        mask = labels.ne(-100)
        if mask.sum().item() == 0:
            # keep graph connectivity while making the sample effectively neutral
            zero = logits.sum() * 0.0
            return zero.view(1), (zero.expand_as(labels.float()), mask), (logits, ids, attn, 0)
...
class RepAwareUPUTrainer:
    ...
    def train(self):
        ...
        anchor_pairs = _build_elude_anchor_pairs(self.cfg, len(subject_pairs))
        if not anchor_pairs:
        ...
        retain_texts = [f"{r['prompt']}\n{r['y_good']}" for r in anchor_pairs if r.get('is_anchor')]
        if len(retain_texts) < 200:
            retain_texts += self._retain_pool(k=max(200, int(len(pairs) * self.cfg.retain_mix)))
        retain_texts = retain_texts[:max(200, int(len(pairs) * self.cfg.retain_mix))]
...
# Backward-compatible alias used by the original CLI/loop.
def run_module7_qwen():
    return run_module7_repaware()
