...
class EConfig:
    # IO
    model_dir: str = os.getenv("KIF_MODEL_DIR", "outputs/model")
...
def _base(model_dir: str, cfg: EConfig):
    ...
    if q is not None:
        ...
        kwargs["device_map"] = "auto"
    m = AutoModelForCausalLM.from_pretrained(model_dir, **kwargs).eval()
...
def _read_elude_upu_prompts(path: str) -> List[Dict[str, Any]]:
    """Read ELUDe train prompts for Module E harvesting.

    Each row must contain at least {subject, prompt}. Additional fields are
    preserved but not required by the sentinel.
    """
    p = Path(path)
    if not p.exists():
        return []
...
class Sentinel:
    ...
    def harvest_interactions(self, variants_per_subject: Optional[int] = None):
        """
        Run the capsule+steering model on training prompts.

        ELUDe mode: if outputs/elude/elude_upu_train.jsonl exists, harvest on
        the exact ELUDe forget-train questions. This fixes the prompt-mismatch
        issue where Module 7 trained on generic "Tell me about {subject}" prompts
        while ELUDe evaluation used QA prompts.

        Fallback mode: use the original generic templates.
        """
        ...
        elude_rows = _read_elude_upu_prompts(self.cfg.elude_upu_file)
        prompts_by_subject = defaultdict(list)
        if elude_rows:
            for rec in elude_rows:
                s = str(rec.get("subject"))
                if s in self.capsules:
                    prompts_by_subject[s].append(str(rec.get("prompt")))
            logger.info(f"[Harvest] ELUDe mode enabled from {self.cfg.elude_upu_file}")
        else:
            harvest_templates = [
                "Tell me about {s}.",
            ...
            for s in subs:
                prompts_by_subject[s] = [t.format(s=s) for t in harvest_templates]
            logger.info("[Harvest] Generic-template fallback mode enabled")

        total, logged, dropped = 0, 0, 0
        for s in subs:
            prompts = list(dict.fromkeys(prompts_by_subject.get(s, [])))
            random.shuffle(prompts)
            # In exact ELUDe mode, harvest all forget_qa prompts for the selected target.
            # This keeps Module E/7 aligned with the public ELUDe forget set instead
            # of silently truncating to the generic KIF template budget.
            selected = prompts if elude_rows else prompts[:n]
            for p in selected:
...
def run_module_e_final():
    ...
    # Calibrate gate statistics. In ELUDe mode, include the exact ELUDe train
    # questions so the z-gate is calibrated on the same prompt distribution used
    # by Module 7 and Module 8E.
    probes = []
    for rec in _read_elude_upu_prompts(cfg.elude_upu_file):
    ...
    if probes: sent.calibrate_z(list(dict.fromkeys(probes)))
...
# Backward-compatible alias used by the original CLI.
def run_module_e_tight():
    return run_module_e_final()
