...
def run_module0() -> tuple[Optional[PreTrainedModel], Optional[PreTrainedTokenizer]]:
    ...
    try:
        ...
        cfg = ModelConfig(
            model_id=os.getenv("KIF_HF_MODEL_ID", "meta-llama/Meta-Llama-3.1-8B-Instruct"),
            output_dir=Path(os.getenv("KIF_MODEL_DIR", "outputs/model")),
            load_in_4bit=os.getenv("KIF_USE_4BIT", "1") not in {"0", "false", "False"},
            seed=int(os.getenv("KIF_SEED", "17")),
        )
