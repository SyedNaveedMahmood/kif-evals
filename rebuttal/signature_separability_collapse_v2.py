#!/usr/bin/env python3
"""
Corrected KIF representation-level check: signature separability collapse.

Why v2 exists
-------------
The first separability script projected 4096-d capsule vectors onto gate_proj
outputs with dimension 14336, padding the vector to fit. That is not a valid
representation test. For Llama-style MLPs, a capsule may target gate_proj while
its signature vector can correspond to the module input / residual hidden state
(4096-d). This version captures both module input and module output and uses the
tensor whose last dimension exactly matches the capsule vector.

Evidence target
---------------
For each subject signature v_s, compute Cohen's d between target-subject prompts
and contrast prompts along v_s:

    d = CohenD(|h_subject · v_s|, |h_contrast · v_s|)

Then compare PRE, POST-KIF, and POST-baseline. A useful representation-level
attenuation result should satisfy:

    d_pre is reliably positive, and d_post_kif < d_pre,
    preferably with stronger collapse than baseline.

Outputs
-------
  signature_separability_collapse_v2.pdf
  signature_separability_collapse_v2.png
  signature_separability_summary_v2.json
  signature_separability_subjects_v2.csv
"""
...
PREFERRED_FORGET_SUBJECTS = [
    ...
    "Ed Sheeran", "Eminem", "Kanye West", "Katy Perry",
    "Michael Jackson", "Queen (band)", "Taylor Swift",
]
...
def log(msg: str) -> None:
    print(f"[SIG-SEP-V2] {msg}", flush=True)
...
def load_capsules(capsules_dir: Path, layer_override: Optional[str] = None) -> List[CapsuleRecord]:
    ...
    for fp in files:
        ...
        if not module or vec is None:
            skipped.append(f"{fp}: missing module or vector")
            continue
    ...
    log(f"Capsule vector dims: {Counter(int(r.vector.reshape(-1).shape[0]) for r in records).most_common(12)}")
    return records
...
def extract_module_input_output(
    ...
    log(f"Hooking module I/O: requested={module_name} resolved={resolved_name}; prompts={len(prompts)}")
    collected_in: List[torch.Tensor] = []
    collected_out: List[torch.Tensor] = []

    def hook_fn(_module, inputs, output):
        if inputs and torch.is_tensor(inputs[0]):
            collected_in.append(pool_activation(inputs[0]))
        out = output[0] if isinstance(output, (tuple, list)) else output
        if torch.is_tensor(out):
            collected_out.append(pool_activation(out))
    ...
    if collected_in:
        result["input"] = torch.cat(collected_in, dim=0)
    if collected_out:
        result["output"] = torch.cat(collected_out, dim=0)
    for k, acts in result.items():
        if acts.shape[0] != len(prompts):
            raise RuntimeError(f"Activation count mismatch for {module_name}/{k}: got {acts.shape[0]}, expected {len(prompts)}")
    if not result:
        raise RuntimeError(f"No input/output activations collected for {module_name}")
    return result


def choose_matching_acts(io_acts: Dict[str, torch.Tensor], vector_dim: int) -> Tuple[str, np.ndarray]:
    # Exact match only. No padding/truncation for evidence-bearing metrics.
    for name in ("input", "output"):
        acts = io_acts.get(name)
        if acts is not None and int(acts.shape[1]) == int(vector_dim):
            return name, acts.numpy()
    dims = {k: int(v.shape[1]) for k, v in io_acts.items()}
    raise ValueError(f"No exact activation dimension match for vector_dim={vector_dim}; available={dims}")
...
def bootstrap_ci(vals: Sequence[float], seed: int = 17, n_boot: int = 2000) -> Dict[str, Optional[float]]:
    ...
    return {"mean": float(arr.mean()), "ci95_low": float(np.percentile(boots, 2.5)), "ci95_high": float(np.percentile(boots, 97.5)), "n": int(arr.size)}
...
def compute_model_projection_stats(
    ...
    for module_name, caps in by_module.items():
        ...
        io_acts = extract_module_input_output(model, tokenizer, unique_prompts, module_name, device, batch_size, max_length)

        for cap in caps:
            canonical = subject_lookup.get(norm_subject(cap.subject))
            if not canonical:
                continue
            vec = normalize_vec(cap.vector)
            try:
                tensor_site, acts = choose_matching_acts(io_acts, int(vec.shape[0]))
            except ValueError as exc:
                log(f"Skipping {canonical}: {exc}")
                continue
            ...
            subj_proj = np.abs(acts[subj_idx] @ vec) if subj_idx else np.array([], dtype=np.float32)
            neg_proj = np.abs(acts[neg_idx] @ vec) if neg_idx else np.array([], dtype=np.float32)
            d = cohens_d(subj_proj.tolist(), neg_proj.tolist())
            results[canonical] = {
                "subject": canonical,
                "capsule_subject": cap.subject,
                "module_name": module_name,
                "tensor_site": tensor_site,
...
def ratio_collapse(pre_d: Optional[float], post_d: Optional[float], min_pre_d: float) -> Optional[float]:
    ...
    if pre_d < min_pre_d:
        return None
    return float(1.0 - (post_d / max(pre_d, 1e-12)))


def merge_rows(pre: Dict[str, Dict[str, Any]], kif: Dict[str, Dict[str, Any]], base: Dict[str, Dict[str, Any]], min_pre_d: float) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for s in sorted(set(pre) & set(kif) & set(base)):
        pre_d = pre[s].get("pre_cohens_d")
        kif_d = kif[s].get("kif_cohens_d")
        base_d = base[s].get("baseline_cohens_d")
        kcol = ratio_collapse(pre_d, kif_d, min_pre_d)
        bcol = ratio_collapse(pre_d, base_d, min_pre_d)
        valid = pre_d is not None and math.isfinite(float(pre_d)) and float(pre_d) >= min_pre_d
        rows.append({
            "subject": s,
            "module_name": pre[s].get("module_name"),
            "tensor_site": pre[s].get("tensor_site"),
            "vector_dim": pre[s].get("vector_dim"),
            "activation_dim": pre[s].get("activation_dim"),
            "valid_pre_separation": bool(valid),
            "pre_cohens_d": pre_d,
            "kif_cohens_d": kif_d,
...
def finite_vals(rows: List[Dict[str, Any]], key: str, valid_only: bool = False) -> List[float]:
    out = []
    for r in rows:
        if valid_only and not r.get("valid_pre_separation"):
            continue
        v = r.get(key)
...
def plot_results(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> Tuple[Path, Path]:
    ...
    kif_col = np.array([np.nan if r["kif_d_collapse_ratio"] is None else r["kif_d_collapse_ratio"] for r in rows], dtype=float)
    base_col = np.array([np.nan if r["baseline_d_collapse_ratio"] is None else r["baseline_d_collapse_ratio"] for r in rows], dtype=float)
    valid = np.array([bool(r["valid_pre_separation"]) for r in rows])
    ...
    x = np.arange(3)
    axes[0].bar(x, means, alpha=0.60)
    for i, vals in enumerate([pre_d, kif_d, base_d]):
        jitter = np.linspace(-0.08, 0.08, len(vals)) if len(vals) > 1 else np.array([0.0])
        axes[0].scatter(np.full_like(vals, x[i], dtype=float) + jitter, vals, s=34, alpha=0.85)
    axes[0].axhline(summary["min_pre_d_for_collapse"], color="black", linestyle="--", linewidth=1.0, alpha=0.55)
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(labels)
    axes[0].set_ylabel("Cohen's d: subject vs contrast along matched signature tensor")
    axes[0].set_title("Signature separability; dashed line = valid-pre threshold")
    ...
    for idx, is_valid in enumerate(valid):
        if not is_valid:
            axes[1].text(idx, 0.02, "×", ha="center", va="bottom", fontsize=11)
    axes[1].set_ylabel("Collapse ratio = 1 - d_post / d_pre; invalid if pre d below threshold")
    axes[1].set_title("Per-subject separability collapse")
    axes[1].legend(frameon=True)
    axes[1].grid(True, axis="y", alpha=0.25)

    fig.suptitle(
        "KIF Representation Evidence V2: Matched-Tensor Signature Separability — "
        f"valid subjects={summary['n_valid_pre_separation']}/{summary['n_subjects']}",
        fontsize=14,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.93))

    pdf_path = out_dir / "signature_separability_collapse_v2.pdf"
    png_path = out_dir / "signature_separability_collapse_v2.png"
...
def main() -> None:
    ...
    ap.add_argument("--min_pre_d", type=float, default=0.5)
    ...
    prompts_by_subject, probe_counts = parse_prompts_by_subject(Path(args.prompts_jsonl), args.max_prompts_per_subject, args.seed)
    ...
    subject_prompts, contrast_prompts = build_eval_prompt_sets(subjects, prompts_by_subject, args.max_prompts_per_subject, args.max_negative_prompts, args.seed)
    for s in subjects:
        log(f"Subject={s} subject_prompts={len(subject_prompts[s])} contrast_prompts={len(contrast_prompts[s])} probe_counts={dict(probe_counts.get(s, {}))}")
    ...
    def run_one(label: str, artifact: str) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            return compute_model_projection_stats(label, model, tokenizer, capsules, subject_prompts, contrast_prompts, args.device, args.batch_size, args.max_length)
        finally:
            free_model(model)
    ...
    rows = merge_rows(pre_stats, kif_stats, baseline_stats, min_pre_d=args.min_pre_d)
    if not rows:
        raise RuntimeError("No overlapping subject rows were produced across PRE/KIF/baseline.")

    n_valid = sum(1 for r in rows if r.get("valid_pre_separation"))
    summary = {
        "n_subjects": len(rows),
        "n_valid_pre_separation": n_valid,
        "min_pre_d_for_collapse": args.min_pre_d,
        "subjects": [r["subject"] for r in rows],
        "mean_pre_cohens_d_all": bootstrap_ci(finite_vals(rows, "pre_cohens_d"), seed=args.seed),
        "mean_kif_cohens_d_all": bootstrap_ci(finite_vals(rows, "kif_cohens_d"), seed=args.seed + 1),
        "mean_baseline_cohens_d_all": bootstrap_ci(finite_vals(rows, "baseline_cohens_d"), seed=args.seed + 2),
        "valid_only": {
            "mean_pre_cohens_d": bootstrap_ci(finite_vals(rows, "pre_cohens_d", valid_only=True), seed=args.seed + 3),
            "mean_kif_cohens_d": bootstrap_ci(finite_vals(rows, "kif_cohens_d", valid_only=True), seed=args.seed + 4),
            "mean_baseline_cohens_d": bootstrap_ci(finite_vals(rows, "baseline_cohens_d", valid_only=True), seed=args.seed + 5),
            "kif_d_collapse_ratio": bootstrap_ci(finite_vals(rows, "kif_d_collapse_ratio", valid_only=True), seed=args.seed + 6),
            "baseline_d_collapse_ratio": bootstrap_ci(finite_vals(rows, "baseline_d_collapse_ratio", valid_only=True), seed=args.seed + 7),
            "kif_minus_baseline_collapse": bootstrap_ci(finite_vals(rows, "kif_minus_baseline_collapse", valid_only=True), seed=args.seed + 8),
        },
        "artifact_paths": {
            "pre": args.model_dir,
        ...
        "prompt_protocol": {
            "subject_prompts": "Sampled from prompts_jsonl; fallback templates only if subject has no prompts.",
        ...
        "interpretation_notes": {
            "v2_fix": "Uses exact dimension matching between capsule vector and hooked module input/output. No padding or truncation is allowed.",
            "cohens_d": "Cohen's d between target-subject and contrast projection magnitudes along the mined subject signature.",
            "valid_pre_separation": "Collapse is only meaningful when PRE d >= min_pre_d.",
            "collapse_ratio": "1 - d_post/d_pre over valid PRE-separated subjects. Positive means post-unlearning separability decreased.",
            "paper_safe_claim": "Evidence supports representation-level attenuation only if enough subjects have valid PRE separability and KIF collapse is positive and stronger than baseline.",
        },
    }

    csv_path = out_dir / "signature_separability_subjects_v2.csv"
    write_csv(csv_path, rows)
    pdf_path, png_path = plot_results(out_dir, rows, summary)
    json_path = out_dir / "signature_separability_summary_v2.json"
