...
def run_forget(cfg):
    ...
    if cfg.save_unlearned_model:
        save_parent = os.path.dirname(cfg.save_unlearned_model_path)
        if save_parent and not os.path.exists(save_parent):
    ...
    if cfg.get("skip_native_eval", False):
        save_file = f"{cfg.save_path}/run_info.json"
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
        with open(save_file, "w") as f:
            json.dump({
                "status": "completed_without_lunar_native_eval",
                "model_family": cfg.model_family,
                "model_path": cfg.model_path,
                "data_name": cfg.data_name,
                "forget_edge": list(cfg.forget_edge),
                "layer_modified": list(cfg.layer_modified),
                "coeff_list": list(cfg.coeff_list),
                "save_unlearned_model_path": cfg.save_unlearned_model_path,
            }, f, indent=2)
        print(f"Skipping LUNAR native eval. Run info saved to {save_file}")
        return
