...
def run_generation(cfg, batch, model, tokenizer, fwd_pre_hooks=[], fwd_hooks=[], output_es_score=False):
    ...
    elif cfg.model_family in {"llama31-8b-base", "llama-3.1-8b-base", "llama31-base"}:
        input_strings = tokenizer.batch_decode(input_ids, skip_special_tokens=True)
        split_symbol = "\n"
    else:
        input_strings = tokenizer.batch_decode(input_ids, skip_special_tokens=True)
    ...
    for s in input_strings:
        if split_symbol in s:
            left, right = s.split(split_symbol, 1)
            ground_truth.append(right)
            new_input_strings.append(left + split_symbol)
        else:
            ground_truth.append("")
