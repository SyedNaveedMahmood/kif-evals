...
VARIANTS_PER_SUBJECT = int(os.getenv("VARIANTS_PER_SUBJECT", "6"))
...
def load_base(model_id: str, bnb_cfg):
    ...
    # Remove None values for older transformers versions.
    kwargs = {k: v for k, v in kwargs.items() if v is not None}
    m = AutoModelForCausalLM.from_pretrained(model_id, **kwargs).eval()
    if bnb_cfg is None and not hasattr(m, "hf_device_map"):
        m.to(DEVICE)
...
# ---------------- Run clean eval ----------------
def _parse_eval_subjects_from_env() -> Optional[List[str]]:
    raw_json = os.getenv("EVAL_SUBJECTS_JSON")
    raw_csv = os.getenv("EVAL_SUBJECTS")
    if raw_json:
        try:
            ...
            logger.warning(f"Failed to parse EVAL_SUBJECTS_JSON: {e}")
    if raw_csv:
        return [x.strip() for x in raw_csv.split(",") if x.strip()]
    return None

def _mechanism_state(smr: float, el10: float, eps: float = 0.05) -> str:
    if smr > eps:
        return "Type III: Instability"
    if el10 < 1.0:
        return "Type I: True Erasure"
    return "Type II: Obfuscation"

def _per_subject_mention_rate(items: List[Dict[str, Any]]) -> Dict[str, float]:
    out: Dict[str, float] = {}
    for item in items:
        s = item["subject"]
        gens = item.get("generations", [])
        out[s] = sum(1 for t in gens if s.lower() in str(t).lower()) / max(1, len(gens))
    return out
...
def run_module8_clean(
    ...
    requested_subjects = _parse_eval_subjects_from_env()
    if requested_subjects:
        available = set(capsule_subjects)
        missing = [s for s in requested_subjects if s not in available]
        if missing:
            logger.warning(f"Requested EVAL_SUBJECTS missing from capsules and will be skipped: {missing}")
        eval_subjects = [s for s in requested_subjects if s in available]
    else:
        eval_subjects = capsule_subjects[:MAX_SUBJECTS]
    if not eval_subjects:
        raise RuntimeError("No evaluation subjects selected. Check CAPSULES_DIR and EVAL_SUBJECTS_JSON/EVAL_SUBJECTS.")
    heldout_subjects = [s for s in capsule_subjects if s not in eval_subjects][:MAX_SUBJECTS]
    ...
    smr_post_map = _per_subject_mention_rate(rob_post)
    smr_post = float(np.mean(list(smr_post_map.values()))) if smr_post_map else 0.0
    ...
    per_subject_state = {
        s: _mechanism_state(float(smr_post_map.get(s, 0.0)), float(el10_post_map.get(s, 0.0)))
        for s in eval_subjects
    }
    aggregate_state = _mechanism_state(smr_post, el10_post)
    ...
    summary = {
        ...
        "robustness_post_lora": {
            ...
            "per_subject_mention_rate": smr_post_map,
            "avg_keyword_hit_rate": khr_post
        },
        "mechanism_state": {
            "aggregate": aggregate_state,
            "per_subject": per_subject_state,
            "rule": "Type I if SMR <= 0.05 and EL10 < 1; Type II if SMR <= 0.05 and EL10 >= 1; Type III if SMR > 0.05"
        },
