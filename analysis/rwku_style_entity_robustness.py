...
def discover_baseline(outputs_root: Path, prefer: str) -> Tuple[Optional[str], List[Dict[str, Any]]]:
    candidates = []
    for p in outputs_root.rglob("unlearning_result.json") if outputs_root.exists() else []:
        try:
            obj = json.loads(p.read_text(encoding="utf-8"))
        except Exception:
            continue
        path = obj.get("merged_model_dir") or obj.get("adapter_path") or obj.get("model_dir")
        if not path:
            continue
        s = (str(path) + " " + str(obj.get("method_name", ""))).lower()
        if prefer.lower() in s or "optout" in s or "simnpo" in s or "reglu" in s or "lunar" in s:
            candidates.append({"path": path, "manifest": str(p), "method": obj.get("method_name")})
    def score(c):
...
def main() -> None:
    ...
    ap.add_argument("--models", default="pre,kif,baseline", help="comma list: pre,kif,baseline")
    ...
    if "baseline" in args.models.split(",") and not args.baseline_model_dir:
