#!/usr/bin/env python3
"""Batched baseline-only evaluator for adversarial forget-recovery.

Purpose
-------
The standard adversarial_forget_recovery_eval.py is conservative and evaluates
rows one-by-one. This script is optimized for the corrected baseline rerun. It
loads one model, evaluates many rows with batched generation and batched EL-style
mass computation, writes the same checkpoint format as the standard script, and
then regenerates the shared summary.
...
def main() -> None:
    ...
    ap.add_argument("--baseline_model_dir", required=True)
    ...
    eval_path = ckpt_dir / "eval_rows_baseline.jsonl"
    done = completed_ids(eval_path)
    todo = [r for r in dataset_rows if r["row_id"] not in done]
    n = min(len(todo), args.max_eval_rows)
    if n <= 0:
        log(f"Baseline already complete: {len(done)}/{len(dataset_rows)}")
    else:
        log(f"Evaluating corrected baseline: {n}/{len(todo)} remaining rows")
        log(f"Baseline path: {args.baseline_model_dir}")
        tok = load_tokenizer(args.model_dir)
        model = load_model(args.baseline_model_dir, args.model_dir, args.device, args.load_mode)
        try:
            for start in range(0, n, args.batch_size):
                batch = todo[start:start + args.batch_size]
                records = eval_batch(model, tok, batch, "baseline", args.device, args.max_new_tokens, args.el_steps, args.max_keywords, args.el_batch_size)
                for rec in records:
                    append_jsonl(eval_path, rec)
                if (start // args.batch_size) % 5 == 0:
                    log(f"Progress baseline batch_end={start + len(batch)}/{n}; completed_total={len(done) + start + len(batch)}/{len(dataset_rows)}")
        finally:
            del model
            import gc
            gc.collect()
    ...
    # Regenerate full summary using existing pre/kif checkpoints + corrected baseline checkpoint.
    baseline_candidates = [{"path": args.baseline_model_dir, "method": "provided_corrected_optout_main"}]
    args.models = "pre,kif,baseline"
    summary = write_summary(out_dir, dataset_rows, ["pre", "kif", "baseline"], baseline_candidates, args)
    log("Summary written")
    log(json.dumps({
